...
public class ButtonGenerator {
    ...
    private String getButtonLabel(String fileName) {
        ...
        if (parts.length >= 2) {
            ...
            int dotIndex = labelWithExtension.lastIndexOf(".");
            if (dotIndex >= 0) {
                // 返回原始的文件名部分，不进行大写转换
                return labelWithExtension.substring(0, dotIndex);
            }
        }
        return null;
    }
    ...
    private Drawable getButtonIcon(String fileName) {
        ...
        if (parts.length >= 1) {
            ...
            try {
                ...
                int resourceId = context.getResources().getIdentifier("image" + sorting, "mipmap", context.getPackageName());
                if (resourceId != 0) {
    ...
    private void onButtonClick(Button button) {
            if (selectedButton != null) {
                ...
                //设置未选中状态文字颜色
                selectedButton.setTextColor(ContextCompat.getColor(context, android.R.color.black));
                //设置未选中状态文字普通粗细
            ...
            //设置选中状态文字颜色
            selectedButton.setTextColor(ContextCompat.getColor(context, R.color.button_selected_text_color));
            //设置选中状态文字加粗
